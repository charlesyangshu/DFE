...
blk_sz = 2;
im_pth = '../../../Dataset/pristinedata/pristine/';
current = pwd;
cd(sprintf('%s',im_pth))
names        = ls;
names        = names(3:end,:);
...
for idx_im = 1:size(names,1)
    ...
    %% Gradient
    IxY1 = conv2(O1, dx, 'same');
    IyY1 = conv2(O1, dy, 'same');
    O1 = sqrt(IxY1.^2 + IyY1.^2);
    
    IxY1 = conv2(O2, dx, 'same');
    IyY1 = conv2(O2, dy, 'same');
    O2 = sqrt(IxY1.^2 + IyY1.^2);
    
    IxY1 = conv2(O3, dx, 'same');
    IyY1 = conv2(O3, dy, 'same');
    O3 = sqrt(IxY1.^2 + IyY1.^2);
    %% im2blk
...
kernel{3} = [v0, vt];
%%
save(['Kernels/PCA_kernel_G_x',num2str(blk_sz),'_bmp'],'-v7.3','kernel');